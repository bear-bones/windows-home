...
/* {{{ tabSecurity(OUTPUT w-tab-list, OUTPUT w-hide-list) */
PROCEDURE tabSecurity:
    ...
    DO i = 1 TO NUM-ENTRIES(w-tab-dats):
        IF hasSecurity(s-user-id, "%%SHORTSYS%%", ENTRY(i, w-tab-dats), 'R')
        THEN
            w-tab-list = w-tab-list + ',' + STRING(i).
        ELSE
            w-hide-list = w-hide-list + w-prefix + STRING(i) + "@)".
    END.
