...
" for, as in "define input-output parameter table for" is an operator unless it
" is the first word on the line
syn match ProgressNormalFor	    "\<for\>"
syn match ProgressTableFor		"\<table\([ \t\r\n]\+for\|\)\>"
...
syn region ProgressProcedureBlock	transparent matchgroup=ProgressProcedure start="\<procedure\>" matchgroup=ProgressProcedure end="\<end\sprocedure\>\|\<end\>" contains=ALLBUT,ProgressProcedure,ProgressFunction
syn region ProgressFunctionBlock	transparent matchgroup=ProgressFunction start="\<function\>" matchgroup=ProgressFunction end="\<end\sfunction\>\|\<end\>" contains=ALLBUT,ProgressProcedure,ProgressFunction
...
syn keyword ProgressOOP         class interface
syn keyword ProgressFunction	function returns
syn keyword ProgressProcedure	procedure

syn keyword ProgressReserved	abstract accum[ulate] active-window add alias all alter ambig[uous] analyz[e] any apply as asc[ending] assign at attr[-space]
...
syn keyword ProgressReserved	cpstream create ctos current-changed current-lang[uage] current-window current_date curs[or] database dataservers day
...
syn keyword ProgressReserved	disp[lay] distinct dos down drop editing enable encode end-error entry error-stat[us] escape etime except exclusive
syn keyword ProgressReserved	exclusive[-lock] exclusive-web-us[er] exists export fetch field field[s] file-info[rmation] fill final find-case-sensitive
syn keyword ProgressReserved	find-global find-next-occurrence find-prev-occurrence find-select find-wrap-around first-of focus font form form[at]
...
syn keyword ProgressReserved	go-pend[ing] grant graphic-e[dge] group having header help hide implements import in index indicator inherits input input-o[utput] insert
...
syn keyword ProgressReserved	map member message message-lines month mouse mpe new next-prompt no-attr[-space] no-error no-f[ill] no-help no-hide no-label[s]
syn keyword ProgressReserved	no-lock no-map no-mes[sage] no-pause no-prefe[tch] no-undo no-val[idate] no-wait null num-ali[ases] num-dbs num-entries
syn keyword ProgressReserved	of off old on open opsys os-append os-command os-copy os-create-dir os-delete os-dir os-drive[s] os-error os-rename
...
syn keyword ProgressReserved	stream-io string-xref system-dialog term term[inal] text text-cursor text-seg[-growth] this-procedure time title
syn keyword ProgressReserved	to today top-only trans trans[action] trigger triggers trim underl[ine] undo unform[atted] union unique unix up update
...
syn keyword ProgressReserved	window window-maxim[ized] window-minim[ized] window-normal with work-tab[le] workfile write xcode xref year _cbit
...
syn match  ProgressBoolean		"\<\(true\|false\|yes\|no\)\>"
...
syn keyword ProgressOperator	abs[olute] accelerator across add-first add-last advise alert-box allow-replication ansi-only and anywhere append appl-alert[-boxes] application as-cursor ask-overwrite
...
syn keyword ProgressOperator	border-right-pix[els] border-t[op] border-t[op-chars] border-top-pixel[s] both bottom box box-select[able] browse browse-header buffer buffer-chars buffer-compare buffer-copy buffer-lines
syn keyword ProgressOperator	button button[s] byte cache cache-size can-query can-set cancel-break cancel-button caps careful-paint case-sensitive cdecl character_length charset
...
syn keyword ProgressOperator	current-result-row current-row-modified current-value cursor-char cursor-line cursor-offset data-entry-retur[n] data-t[ype] date-f[ormat] db-references
syn keyword ProgressOperator	dcolor dde-error dde-i[d] dde-item dde-name dde-topic debu[g] default-b[utton] default-extensio[n] defer-lob-fetch defined delete-char delete-current-row
syn keyword ProgressOperator	delete-line delete-selected-row delete-selected-rows deselect-focused-row deselect-rows deselect-selected-row design-mode dialog-box dialog-help dir disabled display-message
...
syn keyword ProgressOperator	fore[ground] form-input forwards frame-spa[cing] frame-x frame-y frequency from-cur[rent] full-height full-height-char[s] full-height-pixe[ls] full-pathn[ame]
...
syn keyword ProgressOperator	initial-filter initiate inner inner-chars inner-lines insert-b[acktab] insert-file insert-row insert-string insert-t[ab] internal-entries is-lead-byte
...
syn keyword ProgressOperator	load-mouse-point[er] load-small-icon lookahead lower lt manual-highlight margin-extra margin-height margin-height-ch[ars] margin-height-pi[xels] margin-width
...
syn keyword ProgressOperator	min min-height min-height[-chars] min-height-pixel[s] min-size min-val[ue] min-width min-width[-chars] min-width-p[ixels] min[imum] mod modified mod[ulo] mouse-p[ointer]
...
syn keyword ProgressOperator	native ne new-row next-col[umn] next-sibling next-tab-ite[m] next-value no-apply no-assign no-bind-where no-box no-column-scroll[ing] no-convert no-current-value
...
syn keyword ProgressOperator	none not num-but[tons] num-col[umns] num-copies num-formats num-items num-iterations num-lines num-locked-colum[ns] num-messages num-results num-selected num-selected-rows
syn keyword ProgressOperator	num-selected-widgets num-tabs num-to-retain numeric numeric-f[ormat] octet_length ok-cancel on-frame[-border] or ordered-join ordinal orientation os-getenv outer
...
syn keyword ProgressOperator	position precision presel[ect] prev-col[umn] prev-sibling prev-tab-i[tem] primary printer-control-handle printer-setup private-d[ata] profiler Progress-s[ource]
...
syn keyword ProgressOperator	return-ins[erted] return-to-start-di[r] reverse-from right right-align[ed] right-trim round row row-ma[rkers] row-of rule rule-row rule-y save-as save-file
...
syn keyword ProgressOperator	title-dc[olor] title-fgc[olor] title-fo[nt] to-rowid toggle-box tool-bar top topic total trailing trunc[ate] unbuff[ered] unique-id unload unsubscribe upper use use-dic[t-exps]
...
syn keyword ProgressOperator	widget-pool width width[-chars] width-p[ixels] window-name window-sta[te] window-sys[tem] word-wrap x-of y-of yes-no yes-no-cancel _dcm

syn keyword ProgressReserved    var[iable] char[acter] int[eger] log[ical] rowid recid date format dec[imal]
...
syn keyword ProgressConditional	if then do else end repeat transaction while each where procedure function
syn keyword ProgressConditional forward otherwise when find first last next prev of get returns current
